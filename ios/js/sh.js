...
(function () {
    ...
    // ✅ 判断是否为搜狐移动文章链接，如 /a/916074732_114988
    if (!/\/a\/\d+_\d+/.test(location.pathname)) {
        console.log('[Readability] 非搜狐文章页面，跳过执行');
        return;
    }
    ...
    function applyReadability() {
        try {
            ...
            // 可选：强制只保留正文容器再传给 Readability，提高准确度
            const articleContainer = docClone.querySelector('.article, .text, .text-box, .article-box');
            if (articleContainer) {
                docClone.body.innerHTML = articleContainer.outerHTML;
            }
    ...
    // ✅ 等待正文出现后再触发阅读模式（可适配动态加载内容）
    const observer = new MutationObserver(() => {
        if (document.querySelector('.article, .text, .text-box, .article-box, .text-content')) {
            observer.disconnect();
            setTimeout(applyReadability, 800); // 可按情况加大延迟
        }
    });
