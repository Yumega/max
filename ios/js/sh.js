// ==UserScript==
// @name         Sohu 移动版自动阅读模式（优化版）
// @namespace    http://tampermonkey.net/
// @version      1.0
// @description  自动提取并美化搜狐移动站文章正文，仅对 URL 含 /a/数字_数字 生效，延迟执行确保完整内容提取。
// @author       ChatGPT
// @match        https://m.sohu.com/a/*
// @grant        none
// ==/UserScript==

/* ——— 在此粘贴 Readability.js 的完整源码 ——— */
/*
 * available at: https://raw.githubusercontent.com/mozilla/readability/refs/heads/main/Readability.js
 */
...
(function () {
    ...
    function applyReadability(sourceNode) {
        const docClone = document.implementation.createHTMLDocument("clone");
        docClone.body.innerHTML = sourceNode.outerHTML;
        ...
        style.textContent = `
            #readability-content p { margin: 1.2em 0; }
            #readability-content img, video, iframe {
                max-width: 100%; display: block; margin: 1.5em auto;
            }
            #readability-content h2 {
                font-size: 1.2em; margin-top: 2em; font-weight: bold;
            }
    ...
    const observer = new MutationObserver(() => {
        const articleNode = document.querySelector('.text');
        if (articleNode && articleNode.innerText.length > 100) {
            observer.disconnect();
            applyReadability(articleNode);
        }
    });
